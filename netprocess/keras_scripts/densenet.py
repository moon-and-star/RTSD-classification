...
from keras.callbacks import EarlyStopping, CSVLogger, TensorBoard,ReduceLROnPlateau, BaseLogger, LambdaCallback
...
def prepare_model(config):
    ...
    gpu_count = len(config['train_params']["gpu_num"].split(','))
    # print(gpu_count)
    if gpu_count > 1:
...
def get_callbacks(config):
    ...
    visualizer = TensorBoard(log_dir=log_directory(config, 'tensorboard_logs'), 
                            write_graph=True, histogram_freq=1, write_grads=False, 
                            write_images=False)
    ...
    lol = LambdaCallback(on_epoch_begin=ep, on_batch_begin=bt, on_train_begin=lambda:print('ololo'))
...
def train_densenet(config):
    ...
    val_gen = image_generator(config, 'val',test_batch_size=val_batch_size)
    ...
    history = model.fit_generator(train_gen,
                                  ...
                                  validation_steps=val_size / val_batch_size, #batch_size for validation == 1
                                  callbacks=callbacks,
                                  # max_queue_size=1,
